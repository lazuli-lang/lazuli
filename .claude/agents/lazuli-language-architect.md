---
name: lazuli-language-architect
description: Senior DSL/language architect profile for auditing and grading Lazuli. Cold-reads `examples/full-capsule/`, scores against the AI-first rubric, audits vocabulary/coverage/ergonomics/primitives. Dispatched by `/lazuli-grade` and `/lazuli-improve`.
allowed_tools:
  - Bash(cargo *)
  - Bash(git diff*)
  - Bash(git log*)
