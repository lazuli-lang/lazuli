...
- **STATUS:** **closed** (lazuli commit pending 2026-05-16) — analyzer now lowers `query.list filters` lines into typed `ir::Filter` predicates; bare single-segment identifiers on the RHS lift to `Expr::Enum(EnumLiteral { type_name: None, variant })` which codegen emits as `lazuli.FromConst("<variant>")` for a TEXT bind. Tests at `crates/lazuli_analyzer/src/lib.rs` (7 unit tests covering dotted paths, booleans, integers, strings, nil, inequality operators, blank/comment skip, and the bare-identifier→enum lift).
- **Original symptom:** declarative form `query.list pending_intermediation_hosts { filters { status = pending } }` parsed cleanly but the analyzer dropped the filter lines entirely (`ir::ListQuery.filters = Vec::new()` in `lower_query_decl`); ALL filter types were unwired — boolean equality (`is_public = false`) silently emitted no `Filters:` block on the runtime side. Hostpoint snapshot's "boolean works, enum doesn't" was overgenerous; nothing worked declaratively. Hostpoint compensated via hand-rolled `command list_* returns JSON` handlers that ran the right SQL by hand.
- **Fix:** `crates/lazuli_analyzer/src/lib.rs` `lower_query_filter_lines()` + `parse_query_filter_line()` + `filter_rhs_expr()`. Lifts each verbatim line via the same operator-split harness used by `parse_closed_predicate`, but the RHS expression promoter treats bare single-segment identifiers as enum variants (vs `Expr::Path` in eval-block predicates where bare identifiers are local column references). Wired into `lower_query_decl` `List` branch; `Lookup` filters remain dropped at parse time per their existing grammar contract.
- **End-to-end validation:** Hostpoint regen post-fix shows 17 queries now emit proper `Filters: []lazuli.FilterRule{...}` blocks (was 0). Specifically `operations.mine_open_transactions_as_traveler` with `status = requested` lowers to `{Column: "status", When: lazuli.FromConst("requested")}` — the WAR closure path. Hand-rolled `command list_*` handlers can be retired once a follow-up Hostpoint refactor pass migrates the storybook screens to the new declarative queries.
- **Follow-up:** doctor lint candidate `VOCAB-QUERY-ENUM-UNKNOWN-001` to reject filter RHS identifiers that don't resolve to a known enum variant of the LHS field's type — currently the analyzer accepts any bare identifier and defers resolution to runtime (Postgres will error if the variant text doesn't match the column's enum). Tracked separately.
- **Surfaced by:** Hostpoint Phase 3.7 (host-operations agenda); closed lazuli 2026-05-16.
