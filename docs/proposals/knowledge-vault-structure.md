...
O setor é o slug que aparece em `knowledge <sector>` (gramática) **e** como pasta `.lazuli/knowledge/<sector>/` (arquivo). O catálogo é **fechado e opinativo** — herdando a Rule Zero ("vocabulário sobre mecanismo"): conhecimento de vocabulário aberto vira dialeto por projeto, exatamente o que o `.lzi` proíbe no domínio (`knowledge-primitive-and-specialist-harness.md:52`).
...
| `specs` | pauta `PRD.md` + `TECH-SPEC.md` (31× cada) + Pleiades `doc` | Requisito (PRD: users/roles, user stories Gherkin, out-of-scope) e/ou esboço técnico (TECH-SPEC: blocos `lazuli`, stubs Go, migration notes) | **PRD**, **TECH-SPEC** |
| `changes` | pauta `changes/<NNN>/` (a unidade de trabalho) | Uma unidade de mudança rastreável, com lifecycle de status próprio (§4) | **CHANGE** (board row + bundle) |
...
`prompt` e `doc` de Pleiades **não viram setor próprio** aqui: `doc` é absorvido por `specs`/`lazuli-way`; `prompt` é matéria de outro produto (harness), não do vault de conhecimento do projeto. (Pleiades v2 já cortou `marketing`/`media` por raciocínio idêntico — `item.lzi:48-50`.)
...
- Adicionar um setor = criar a pasta `.lazuli/knowledge/<sector>/` **e** referenciá-la via `knowledge <sector>` em ≥1 feature. As duas coisas são amarradas por `VOCAB-KNOWLEDGE-SECTOR-UNKNOWN-001` (grammar ↔ file).
...
cites:                                        # = EvidenceRef[] (item.lzi:33-46); cada cite tem um kind FECHADO
  ...
  - { kind: item,  ref: "specs/0003-agency-prd" }
...
- O `NNNN` é o **join key universal** (lição mais forte de pauta): mesmo slug atravessa nome de arquivo, `supersedes:`, `cites:{kind:item}`, e o board. Um `change` e seus `specs` compartilham o `NNNN` (ex.: `0003` cobre o ADR em `decisions/`, o PRD e o TECH em `specs/`).
...
|---|---|---|
| **História append-only, datada** | **git** (`git log` de `.lazuli/knowledge/`) | toda mudança datada/assinada/imutável. Tamanho da história ≠ tamanho do contexto. |
...
### 4.2 `specs` — PRD e TECH-SPEC como item shapes

`specs` carrega dois *shapes* que compartilham o `NNNN` do change (co-locação por ordinal, não por pasta — diferente de pauta que co-locava na mesma pasta `NNN/`):
...
ADR/PRD/TECH-SPEC são, portanto, **item shapes** (templates de corpo) dentro dos setores `decisions`/`specs`, não tipos de primeira classe. O *shape* é convenção de template; o *setor* + *tier* são o que o doctor enxerga.
