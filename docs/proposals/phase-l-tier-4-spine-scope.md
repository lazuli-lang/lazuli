...
| `collect_policy_atoms` | `crates/lazuli_cli/src/doctor.rs:1290-1326` | `policies` block + per-rule `@role.*` / `@scope.*` / `@actor.*` atoms. | **Fully retired (Tier 4 fourth wave, 2026-05-11).** Doctor side retired in W1 via `parse_policies_decl` + `FeatureSkeleton.policies` + `populate_commands_from_ir`. Inspect side retired in fourth wave: `Tier3FeatureSlice.policies: Policies` now feeds `inspect_feature`'s `category -> atoms` lookup, replacing the last text-walker copy in `crates/lazuli_cli/src/main.rs`. |
