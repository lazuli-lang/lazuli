...
**Tracks**: Phase Prep §1.1 of `docs/marketplace-port-checklist.md` — 12-15 cells
...
- `lazuli doctor` learns one new check: `CODEGEN-GO-001` "regenerate `dist/go/` after IR change" — diff `dist/go/<feature>/<feature>.gen.go` header banner against the input file's mtime hash. Out of scope for this proposal; gated until first downstream product port cell ships.
...
- **Sub-shapes**:
  ...
  - `@semantic.GeoPoint` (the downstream product decision §5.1) → emit `postgis.Point` + `db:"… type:geography(point,4326)"` tag (see §9.1).
...
- **Readiness**: List + Lookup **green**. Sql **yellow** — the runtime lib has the `SQL string` field (`query.go:64`) but no spike file uses it; first downstream product port (`reviews.list_by_target` SQL aggregate) exercises it.
...
- **Push channel** (the downstream product §9.5): emitted same shape, adapter resolved through `@adapter.notification.push` → Expo Push plugin (see §6).
...
| `dist/go/migrations/<NNN>_<name>.sql` | DDL migrations derived from `ir::Resource` field set; PostGIS extension toggled by the downstream product §9.1 | N |
...
var lazuliApp = lazuli.AppContract{
    ...
    CORS:    lazuli.AppCors{Allow: []string{"https://marketplace.com.br"}, Credentials: true},
...
dist/go/
  ...
  migrations/
    ...
    002_postgis.sql                    # CREATE EXTENSION IF NOT EXISTS postgis (the downstream product §9.1)
...
**Decision: per-kind file vs one-file-per-feature.** Both are tenable. Recommendation: **per-kind files** (one `resource.gen.go`, one `command.gen.go`, etc.) because (a) it scales to features with 5+ commands without forcing scroll, (b) diff churn localises by kind, (c) matches the downstream product checklist row §1.1's enumeration of `<feature>/<kind>.gen.go`. The spike's monolithic `customer.gen.go` (`dist/go/customer/customer.gen.go`) stays as a transitional fixture and is split on the first emitter cell.
...
## §9. the downstream product decisions (§5 of checklist) — codegen materialisation
