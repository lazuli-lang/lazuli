...
| 24 | Phase L — canonical-indent slice covers commands/resources/queries/records | **prerequisite (Tier 4 outstanding)** | **Repriorizada (2026-05-10).** Tiers 1-3 done: `auth` (Tier 1, commit `e1d8521`), `@cap.File(args)` typing (Tier 2, commit `f60f6bf`), and `job`/`webhook`/`notification`/`event_group` (Tier 3 Route C, commits e89ff27 → 299878e on 2026-05-11). Tier 4 remains: `parse_command` / `parse_resource` / `parse_query` / `parse_record` + lift `defaults.tenancy` from the slice + replace the `JobDeclarative.raw_*` carve-out with the shared declarative spine. After Tier 4 the text-pattern doctor facts for commands/resources can retire. See `docs/proposals/auth-lowering-scope.md`, `docs/proposals/phase-l-tier-3-job-effect-scope.md`. |
