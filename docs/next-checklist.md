...
| 24 | Phase L — canonical-indent slice covers commands/resources/queries/records | **prerequisite** | **Repriorizada (2026-05-10).** The Cut A parser slice covers `agent` only. Commands (used by Cut A.9 `approval`) + resources/records (used by Cut A discriminator records) live in text-pattern doctor facts. Bucket-cycle design pipeline ran on auth/storage/jobs/observability and **3 of 4 buckets independently discovered the same blocker**: surface autorada não chega ao IR porque o slice ignora silenciosamente todos os children que não são `agent`. Phase L deixou de ser tech-debt cut para virar pré-requisito real de rows 26-34 abaixo. See `docs/proposals/auth-lowering-scope.md`, `docs/proposals/bucket-storage-scope.md`, `docs/proposals/bucket-jobs-scope.md`. |
...
| 26 | Auth bucket cycle Route A — canonical-indent slice covers `auth` | planned | Extend `parse_feature_skeleton` to recognise `auth` alongside `agent`; add `parse_auth`; wire `auth: Option<Auth>` through `FeatureSkeleton`. IR extensions: `AuthPassword.algorithm`, `AuthMfa.enroll`/`verify` (gaps found in Stage 4). New `--expand=auth` projection. Depends on row 24 (Phase L). See `docs/proposals/bucket-auth-cycle.md` §Linguagem/§IR + `docs/proposals/auth-lowering-scope.md`. |
...
| 29 | Storage bucket cycle Route B — typed `@cap.File` lowering | planned | Add `FileCapability`/`CapabilityRef`/`FileSize`/`MimeType`/`FileVisibility` to IR. Extend `type_ref_from_syntax` (`crates/lazuli_analyzer/src/lib.rs:486`) to recognise `@cap.File(args)` via the existing `capability_args` helper. New `--expand=storage` projection. Two new args: `visibility`, `signed_ttl`; promoted typing of `max_size`/`accept`. Depends on row 24. See `docs/proposals/bucket-storage-cycle.md` §Linguagem/§IR + `docs/proposals/bucket-storage-scope.md`. |
| 30 | Storage bucket cycle — 5 doctor diagnostics + LSP coverage | planned | `cap_file_visibility_undeclared`, `cap_file_accept_input_output_mismatch`, `cap_file_visibility_signed_ttl_mismatch`, `cap_file_size_unit_invalid` (typed promotion), `cap_file_mime_family_unknown`. LSP hovers for 5 keywords + closed-catalog completions for `visibility`/`max_size`/`signed_ttl`/`accept`. Depends on row 29. See `docs/proposals/bucket-storage-cycle.md` §Doctor/LSP. |
...
| 32 | Jobs/Webhooks/Notifications IR lift (scope-out) | planned | Bring `Job` + `Webhook` IR fields up to fixture surface (`tenant_from`, `fanout`, `timeout`, `external_calls`, `audit`); add `Notification` and `EventGroup` IR structs; wire `InspectFeature.{jobs, webhooks, event_groups}` projections so codegen has typed input. Pre-requisite for the L0→L2 jobs cycle. Depends on row 24. See `docs/proposals/bucket-jobs-scope.md`. |
| 33 | Jobs bucket cycle — L0→L2 closure | planned | Six new IR-driven diagnostics (`JOB-TIMEOUT-001`, `JOB-FANOUT-001/002`, `WEBHOOK-SCOPE-001`, `NOTIF-CHANNEL-001`, `EVENTGROUP-NESTING-001`) + codegen for `dist/go/<feature>/{jobs,webhooks,notifications}.gen.go` + Drusa subpackages `runtime/go/lazuli/{jobs,webhooks,notifications}` + River as primary queue adapter + Sendgrid as primary email adapter. Depends on row 32. See `docs/proposals/bucket-jobs-cycle.md`. |
| 34 | `event_group` doctor pattern-prefix rule | planned | Promote `event_group_can_own_short_event_declarations` LSP rule (`crates/lazuli_lsp/src/lib.rs:14522`) to a doctor cross-feature rule once `EventGroup` IR struct lands. Catches concrete events under a group whose names don't match the pattern prefix. Cheap lift, high value for cold-readability. Depends on row 32. |
| 35 | Observability bucket cycle — 3 new built-in trace events (`command_run` / `job_run` / `webhook_run`) + `@trace.<name>` namespace | planned | **Priorizada após auth** porque desbloqueia observabilidade transversal nos outros buckets. Extends `built_in_trace_events()` from 1 entry (`agent_run`, A.8) to 4. New `TraceFiresPer::CommandDispatch`. New reference namespace `@trace.<name>` for subscriber jobs. 2 doctor diagnostics (`event_trace_reserved_name` extended; `trigger_trace_unknown` new). Runtime instrumentation parallel Drusa work, same pattern as A.8. See `docs/proposals/bucket-observability-cycle.md` §3.5. |
| 36 | Observability bucket cycle — `app.logging` + `app.tracing` blocks | planned | New `AppLogging` + `AppTracing` IR structs on `App`; profile-aware overrides; 6 doctor diagnostics (`app_logging_*`, `app_tracing_*`); LSP hovers + completion for `level`/`format`/`redact`/`sample_rate`/`propagate`. Closed-catalog `level ∈ {debug,info,warn,error}` shared with §3.4. See `docs/proposals/bucket-observability-cycle.md` §Linguagem §IR. |
| 37 | Observability bucket cycle — audit `emit_to` + `event.trace level` + health probe wiring | planned | `Audit.emit_to: Option<String>` resolves to feature event_group or reserved `audit_log`/`audit_stream`. `Event.level` on trace events (closed catalog). Runtime mounts declared `HealthProbes.Liveness`/`Readiness` paths (replaces hardcoded `/healthz` in `runtime/go/lazuli/http.go:28`). 4 doctor diagnostics. Drusa-team owns `runtime/go/lazuli/observability/` package (6 files). See `docs/proposals/bucket-observability-cycle.md` §3.3 §3.4 §Runtime. |
