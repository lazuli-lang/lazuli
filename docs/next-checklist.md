...
| 24 | Phase L — canonical-indent slice covers commands/resources/queries/records | backlog | The Cut A parser slice covers `agent` only. Commands (used by Cut A.9 `approval`) + resources/records (used by Cut A discriminator records) live in text-pattern doctor facts until the slice generalises. No ETA; tracked as a tech-debt cut. |
