...
### Semantic scalars
**Reach for this when you need to:** validate a hex color / percentage / email at the boundary.

`lazuli.HexColor` · `lazuli.Percentage`

> declare the field as the scalar (`color: @semantic.HexColor`) from the closed `@semantic.*` catalog (`@semantic.HexColor`, `@semantic.Percentage`, `@semantic.Email`, `@semantic.Money`); the type validates at decode. Never a regex `^#?[0-9A-Fa-f]{6}$` validator.
