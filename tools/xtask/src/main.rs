...
use xtask::{catalog_reference, docs_staleness, keyword_reference, runtime_surface, tmlanguage};

const USAGE: &str = "usage: cargo xtask <gen-tmlanguage | gen-keyword-reference | \
gen-catalog-reference | gen-runtime-surface | docs-staleness> [--check]";
...
fn main() -> ExitCode {
    ...
    match cmd.as_deref() {
        ...
        Some("gen-runtime-surface") => {
            let check = args.any(|a| a == "--check");
            finish(runtime_surface::run(check))
        }
        Some("docs-staleness") => {
            let check = args.any(|a| a == "--check");
