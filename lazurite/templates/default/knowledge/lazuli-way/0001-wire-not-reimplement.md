...
ecosystem already paid, and it hides bugs the library already fixed. Lean on
typed effects and library adapters rather than bespoke code.
...
discipline (a draft must precede gold in git history) is never bypassed. Each
app may promote it to `gold` once its own usage has battle-tested the rule.
