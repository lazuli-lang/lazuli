...
var (
	...
	// a lock name.
	ErrLockNameRequired = errors.New("jobs: lock name required")
	// ErrLockTTLRequired is returned when a lock acquisition or extension uses
	// a non-positive TTL.
	ErrLockTTLRequired = errors.New("jobs: lock ttl must be positive")
	// ErrLockHeld is returned by non-blocking Acquire calls when another owner
	// currently holds the named lock.
	ErrLockHeld = errors.New("jobs: lock held")
	// ErrLockOwnershipLost is returned when Release or Extend is called with a
	// stale, expired, or non-owner lease.
	ErrLockOwnershipLost = errors.New("jobs: lock ownership lost")
)

// LockToken is an opaque ownership token for a job lock lease.
type LockToken string

// LockLease proves ownership of a named job lock until ExpiresAt.
...
// LockStore is the provider-neutral job distributed lock contract.
//
...
// MemoryLockStore stores job locks in process memory.
//
...
// NewMemoryLockStore returns an empty in-memory job lock store.
func NewMemoryLockStore() *MemoryLockStore {
	return &MemoryLockStore{}
}

// Acquire takes ownership of name for ttl and returns its lease.
//
// Acquire returns ErrLockHeld when name is held by a non-expired lease.
...
func (s *MemoryLockStore) Release(ctx context.Context, lease LockLease) error {
	if err := lockContextErr(ctx); err != nil {
		return err
	}
	if lease.Name == "" {
...
func lockContextErr(ctx context.Context) error {
	if ctx == nil {
		return nil
	}
	return ctx.Err()
