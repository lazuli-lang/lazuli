    ...
    /// `true` when any diagnostic carries `Error` severity — the engine
    /// equivalent of `lazuli doctor` exiting non-zero. Lets engine tests
    /// assert the pass/fail verdict without going through the CLI
    /// `doctor_command` entry layer (which lives in `lazuli_cli`).
    pub(super) fn has_error(diagnostics: &[crate::doctor::DoctorDiagnostic]) -> bool {
        diagnostics
            .iter()
            .any(|d| matches!(d.severity, crate::doctor::DoctorSeverity::Error))
    }
