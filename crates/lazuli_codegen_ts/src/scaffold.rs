//! Static scaffold strings emitted by `generate()` for the frontend
//! starter (package.json / tsconfig / vite config / sample App). Pulled
//! out of `lib.rs` so the crate root stays focused on orchestration.

pub(crate) fn generate_package_json(display_name: &str) -> String {
    format!(
        r#"{{
  "name": "{}",
  "private": true,
...
pub(crate) fn generate_index_html(display_name: &str) -> String {
    format!(
        r#"<!doctype html>
<html lang="en">
  <head>
...
pub(crate) fn generate_tsconfig() -> String {
    r#"{
  "compilerOptions": {
    "target": "ES2022",
...
pub(crate) fn generate_vite_config() -> String {
...
pub(crate) fn generate_main_tsx() -> String {
...
pub(crate) fn generate_app_tsx() -> String {
...
pub(crate) fn generate_styles_css() -> String {
    r#":root {
  color: #172026;
  background: #f6f7f2;
