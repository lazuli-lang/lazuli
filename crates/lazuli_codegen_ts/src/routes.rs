...
fn emit_routes_file(specs: &[RouteSpec]) -> String {
    ...
    // regardless of where it lives in the host filesystem. Pilots path-map
    // both subpaths via tsconfig and bundlers; raw `react` / `@tanstack/*`
    // are not always reachable from the dist file's filesystem location.
    // React types live in @lazuli/runtime/react. TanStack Router primitives
    // (Link, createRoute, etc.) live in @lazuli/runtime/react/tanstack so
    // consumers without TanStack don't pay the bundle cost.
    ...
    s.push_str("  notFoundComponent?: FunctionComponent<any>;\n");
    s.push_str("}) {\n");
