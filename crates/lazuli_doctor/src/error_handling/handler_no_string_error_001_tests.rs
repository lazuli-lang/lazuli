    ...
    // ── FIX 2 — generated extension-stub skip ──────────────────────────

    /// The verbatim Lazuli-generated starter stub (the
    /// `hash_password.go` / `verify_password.go` shape): carries the
    /// `// Code generated by lazuli as a starter stub` header and the
    /// `//lazuli:pattern extension_stub v1` marker, and returns
    /// `errors.New("... not yet implemented")`. An unedited generated
    /// stub can't satisfy an error-handling lint → must be skipped.
    #[test]
    fn generated_extension_stub_file_is_skipped() {
        let f = handler_file(
            "// Code generated by lazuli as a starter stub. It will not be overwritten.\n\
             // IMPLEMENT ME\n\
             package handlers\n\n\
             //lazuli:pattern extension_stub v1\n\
             func HashPassword(ctx *lazuli.Ctx, input string) (string, error) {\n\
             \treturn zero, errors.New(\"hash_password not yet implemented\")\n\
             }\n",
        );
        assert!(
            check(&[f]).is_empty(),
            "an unedited generated extension stub must be skipped"
        );
    }

    /// Per-function guard: a file that mixes a generated stub fn with a
    /// real hand-written handler should skip ONLY the stub fn and still
    /// lint the real one. (Here the file lacks the whole-file generated
    /// header so the file-level fast-path doesn't apply — the
    /// per-function marker tracking carries the skip.)
    #[test]
    fn extension_stub_marker_skips_only_the_marked_function() {
        let f = handler_file(
            "package handlers\n\n\
             //lazuli:pattern extension_stub v1\n\
             func StubFn(ctx *lazuli.Ctx, input string) (string, error) {\n\
             \treturn zero, errors.New(\"stub_fn not yet implemented\")\n\
             }\n\n\
             func RealFn(ctx *lazuli.Ctx, input string) (string, error) {\n\
             \treturn \"\", errors.New(\"hand written ad-hoc\")\n\
             }\n",
        );
        let findings = check(&[f]);
        assert_eq!(
            findings.len(),
            1,
            "only the real hand-written handler should fire, got {findings:?}"
        );
        // The surviving finding is the RealFn one (later line).
        assert!(
            findings[0].line >= 8,
            "finding must be inside RealFn, got line {}",
            findings[0].line
        );
    }
