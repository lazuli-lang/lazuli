...
pub fn check(files: &[RustSourceFile]) -> Vec<Finding> {
    ...
    for file in files {
        ...
        // SPEC-19 split fragments — `<base>_p<N>.rs` / `<base>_impl<N>.rs` (an
        // `include!`d body chunk) and `<base>_tests.rs` (the test sibling) belong
        // to a canonical module and are not independently test-paired. They stay
        // in `library_paths` above, so a `<base>_tests.rs` is still seen as
        // evidence for `<base>.rs`.
        if crate::internal_hygiene::walker::is_split_fragment_name(name) {
            continue;
        }
