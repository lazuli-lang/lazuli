...
fn render_alter_up(
    ...
    if drops_commented || unsafe_present {
        ...
        if drops_commented {
            ...
            .unwrap();
            writeln!(sql, "--   to emit them as live SQL.").unwrap();
        }
        if unsafe_present {
            writeln!(
                sql,
    ...
    if !diff.type_changes.is_empty() {
        ...
        writeln!(sql, "-- TYPE CHANGES").unwrap();
        ...
        for change in &changes {
            if is_safe_cast(&change.previous_sql_type, &change.new_sql_type) {
                writeln!(sql, "{}", render_alter_type_change(change)).unwrap();
            } else {
                writeln!(sql, "-- {}", render_alter_type_change(change)).unwrap();
            }
        }
...
fn render_alter_down(
    ...
    if !diff.type_changes.is_empty() {
        writeln!(sql, "-- TYPE CHANGES (inverse)").unwrap();
        let mut changes = diff.type_changes.clone();
        changes.sort_by(|a, b| a.column.cmp(&b.column));
        for change in &changes {
            if is_safe_cast(&change.previous_sql_type, &change.new_sql_type) {
                writeln!(sql, "{}", render_alter_type_change_down(change)).unwrap();
            } else {
                writeln!(sql, "-- {}", render_alter_type_change_down(change)).unwrap();
            }
        }
    ...
    if !diff.drops.is_empty() {
        ...
        writeln!(sql, "-- DROPS (inverse: re-add)").unwrap();
        let mut drops = diff.drops.clone();
        drops.sort_by(|a, b| a.column.cmp(&b.column));
        for drop in &drops {
            ...
            // executed.
            if options.allow_drops {
                writeln!(sql, "{}", render_alter_drop_down(drop)).unwrap();
            } else {
                writeln!(sql, "-- {}", render_alter_drop_down(drop)).unwrap();
            }
        }
    ...
    if !diff.adds.is_empty() {
        ...
        writeln!(sql, "-- ADDS (inverse: drop)").unwrap();
        let mut adds = diff.adds.clone();
        adds.sort_by(|a, b| a.column.cmp(&b.column));
        for add in &adds {
            writeln!(sql, "{}", render_alter_add_down(add)).unwrap();
        }
    }
