...
use lazuli_ir::{
    ...
    LetBinding, Module, NamedArg, Path, Policies, PolicyRef, QualifiedName, Resource, RetryPolicy,
    TenantFromSpec, TypeRef, UpdateEffect,
};
...
fn scheduled_job_emits_fanout_retry_and_default_policy() {
    ...
    let out = emit(&feature).expect("must emit");
    assert!(out.contains("Trigger:     jobs.JobTrigger{Kind: \"schedule\", Cron: \"0 2 * * *\"},"));
...
fn feature_emit_entry_point_emits_job_file_shape() {
    ...
    assert!(
        out.contains("Trigger:     jobs.JobTrigger{Kind: \"schedule\", Cron: \"*/15 * * * *\"},")
    );
...
fn gated_job_emits_real_prelude_field_and_billing_imports() {
    ...
    let index = CrossFeatureIndex::build(&module);
    let emit_ctx = EmitContext::for_feature(None, "billing-app", "billing", "billing/job.gen.go")
