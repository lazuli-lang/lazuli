...
// ============================================================================
// LAZ-create-table-alter-emit (Cell A11) — ALTER migration emission.
//
// Today every migration emitted above is `CREATE TABLE IF NOT EXISTS`, which
// no-ops on existing databases. Pilots have to author ALTER SQL by hand. The
// pieces below let the orchestrator compute a `SchemaDiff` between the
// baseline (last emitted migration on disk) and the current IR, and emit a
// follow-up `<NNN+1>_<feature>_<resource>_alter.sql` + `.down.sql` pair.
//
// TEMP-STUB notice: cell A10 owns `crates/lazuli_codegen_go/src/emitter/
// schema_diff.rs` (the typed `SchemaDiff` + diff algorithm). At the time
// this cell landed `schema_diff.rs` was NOT yet in tree, so the typed
// shapes below are local stubs. When A10 merges, the orchestrator should
// switch to importing `super::schema_diff::SchemaDiff` (and friends) and
// either re-export the local stubs as aliases or delete them outright. The
// public API of `emit_alter_migration_file` does not need to change — its
