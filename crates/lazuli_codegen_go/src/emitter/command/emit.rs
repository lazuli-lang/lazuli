...
// Test-host siblings — `emit.rs`'s inline `mod tests` is being split
// by sub-concern of the production code. Effect-dispatch tests
// (Creates/Updates/Deletes/Returns/None) moved to
// `emit_effect_dispatch_tests.rs` (wave R8-2c, step 1). The remaining
// inline `mod tests` below holds the binding-source / handler-wrapper
// cluster, scheduled to move to `emit_bindings_and_handlers_tests.rs`
// in the next step of the split.

#[cfg(test)]
mod tests {
    //! Binding-source projection + handler-wrapper tests. Inline
    //! intermediate during wave R8-2c — slated to move to
    //! `emit_bindings_and_handlers_tests.rs` once the second extraction
    //! commit lands.
