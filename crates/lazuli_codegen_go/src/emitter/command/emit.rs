...
#[cfg(test)]
mod tests {
    //! Per-command emission tests — exercise the `emit_command` /
    //! `emit_input_struct` / handler-wrapper sub-pipeline through the
    //! `emit_command_file` orchestrator. Lifted out of `file_emit.rs`
    //! (wave R8-2b) so the per-command concern owns its own tests.
    ...
    use lazuli_ir::{
        ...
        DeleteEffect, EnumLiteral, Expr, InvalidatesSpec, LetBinding, NamedArg, Path, PolicyRef,
        QualifiedName, ReturnsEffect, RouteSlot, TypeRef, UpdateEffect,
    };
