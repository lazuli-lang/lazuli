...
fn app_resolver_registers_each_feature_with_errors() {
    ...
    assert!(out.contains("lazuli.RegisterFeatureErrors(\"account\", accountgen.FeatureErrors)"));
    assert!(out.contains("lazuli.RegisterFeatureErrors(\"billing\", billinggen.FeatureErrors)"));
