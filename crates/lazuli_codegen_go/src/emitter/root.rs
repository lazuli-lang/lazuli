...
pub fn emit_main_go(module: &Module, module_name: &str, source_label: &str) -> String {
    ...
    // the codebase.
    emit_main_imports(&mut p, &feature_imports);
    p.blank();
...
fn emit_main_imports(p: &mut GoPrinter, feature_imports: &BTreeMap<&str, String>) {
    p.line("import (");
    p.indent();
...
mod tests {
    ...
    fn deterministic_main_go_across_runs() {
        ...
        let a = emit_main_go(&module, "lazuli/test-app", "test_app");
        let b = emit_main_go(&module, "lazuli/test-app", "test_app");
        assert_eq!(a, b);
    }
