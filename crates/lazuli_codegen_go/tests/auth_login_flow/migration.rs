//! Go migration helper for the auth_login_flow smoke test. Loaded into
//! the temp output dir as `smoke_migrations/main.go` and invoked via
//! `go run ./smoke_migrations` to apply the emitted `migrations/*.sql`
//! files plus cleanup of any pre-existing rows for the smoke email.
