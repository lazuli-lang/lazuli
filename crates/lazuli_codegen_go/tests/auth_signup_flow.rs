...
mod smoke_e2e {
    ...
    const SMOKE_EMAIL: &str = "new@test.com";
    ...
    impl TempDir {
        fn new(repo_root: &Path) -> Self {
            ...
            let path = repo_root
                .join("target")
                .join("lazuli-go-smoke-auth-signup")
    ...
    #[test]
    fn binary_smoke_auth_signup_flow_sets_session_cookie_and_rejects_duplicate() {
        ...
        let response = post_json(
            port,
            "/auth/signup",
            r#"{"email":"new@test.com","password":"horse-battery-staple","name":"Test"}"#,
        )
        ...
        if response.status != 201 {
            ...
            panic!(
                "POST /auth/signup returned status {}, want 201\nbody:\n{}\nserver output:\n{}",
                response.status, response.body, output
            );
        }
        assert_signup_json_shape(&response.body);
        ...
        .expect("duplicate POST /auth/signup");
        assert_eq!(
            duplicate.status, 409,
            "duplicate POST /auth/signup returned status {}, want 409\nbody:\n{}",
            duplicate.status, duplicate.body
        );
    }
    ...
    fn assert_signup_json_shape(body: &str) {
    ...
    fn assert_session_cookie(response: &HttpResponse) {
        let cookie = response
            ...
            .iter()
            .find(|(name, value)| {
                name.eq_ignore_ascii_case("Set-Cookie") && value.starts_with("lazuli_session=")
            })
            .map(|(_, value)| value.as_str())
            .unwrap_or_else(|| {
                panic!(
                    "response missing lazuli_session Set-Cookie header; headers: {:?}",
                    response.headers
                )
            });

        let value = cookie
            .strip_prefix("lazuli_session=")
            .and_then(|rest| rest.split(';').next())
            .unwrap_or("");
        assert!(
            !value.is_empty(),
            "Set-Cookie lazuli_session value must be non-empty: {cookie}"
        );
        for want in ["HttpOnly", "SameSite=Strict"] {
            assert!(
                cookie.split(';').any(|part| part.trim() == want),
                "Set-Cookie = {cookie:?}, want attribute {want:?}"
            );
        }
    }
...
func main() {
	...
	_, _ = conn.Exec(ctx, `DELETE FROM "user" WHERE email = $1`, "new@test.com")
	_, _ = conn.Exec(ctx, `DELETE FROM "User" WHERE email = $1`, "new@test.com")
}
"#;
}
