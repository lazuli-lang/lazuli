...
";
    let features =
        lazuli_syntax::parse_feature_skeletons(source).expect("parses (annotation is syntactic)");
...
fn validate_sanitize_html_lowers() {
    let constraints = lower_validate_line("validate sanitize_html(basic)").expect("valid profile");
    assert_eq!(
...
fn type_ref_from_syntax_lowers_full_cap_file() {
    let ty = type_ref_from_syntax("@cap.File(max_size:25mb,accept:text/csv,visibility:private)");
    match ty {
