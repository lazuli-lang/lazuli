...
//! `parser.rs` into this directory. The goal is per-source-kind modules
//! so future contributors don't bisect a single megafile when tracking
//! down a parse rule. Stage 2 of W4.4 R2 carved the `.lzx`
//! feature-surface dialect into `lzx.rs`; a follow-up extraction will
//! lift the remaining `.lzi` parsers into `lzi.rs`.
//!
//! ## What lives where
//!
...
//!   `looks_like_policy_expr`, `parse_policy_atom`,
//!   `PolicyExprParser`) consumed by the `.lzi` parsers below.
//! - `mod.rs` (transient) — every `.lzi` parser: `parse_feature_skeletons`,
//!   resources / commands / queries / jobs / webhooks / agents /
//!   notifications / pollers / events / translations / RBAC catalog,
//!   plus the design / plan / package skeleton parsers. Pending
//!   follow-up extraction into `lzi.rs`.
//!
//! ## Cross-module bridges
//!
//! - `parse_invalidates_entry` and `parse_translation_key_token` live
//!   here (with the `.lzi` parsers) and carry `pub(super)` so
//!   `lzx.rs`'s `parse_on_success_block` can call them.
//! - `try_parse_policy_expr` and `looks_like_policy_expr` live in
//!   `lzx.rs` and carry `pub(super)` so every `.lzi` `policy <expr>`
//!   parser can call them.
...
mod policy_expr_parser_tests {
    ...
    use super::lzx::{looks_like_policy_expr, try_parse_policy_expr};
