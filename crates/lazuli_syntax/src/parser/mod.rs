...
//!   crate-private.
//! - `lzx.rs` — `.lzx` app-surface dialect: `parse_lzx_document` plus
//!   the `parse_lzx_<slot>` family for app / route / experience /
//!   surface / audience blocks.
//! - `mod.rs` (transient) — the `.lzx` feature-surface dialect
//!   (`parse_surface_document` + `parse_view_*` + `parse_drawer_*`),
//!   the `.lzi` feature parser (`parse_feature_skeletons`), the
//!   design / plan / package skeleton parsers, and every `.lzi`
//!   per-callable helper. Pending follow-up extractions.
//!
//! ## See also
//!
...
pub use lzx::parse_lzx_document;
...
use crate::ast::{
    ...
    NotificationThrottle, OnSuccessSpecAst, PackageSkeleton,
    ...
    SettingValueSpaceAst, ShadowTokenAst, SortDeclAst, SortDirAst, Span, SqlQueryDecl,
    SqlQueryKind, SurfaceAst, SurfaceTargetAst, TargetArgDecl, TargetExprDecl, TenantMigration,
    ...
    WebhookHandler, WebhookReplay, WebhookVerify, WeightTokenAst, WhenDeniedRouteAst, ZTokenAst,
};
...
fn parse_enum_variant_metadata(
    ...
    for part in split_enum_metadata_commas(metadata) {
        let item = part.trim();
        if item.is_empty() {
            return Err(line_error(line, "enum variant metadata contains an empty item"));
        }
...
fn parse_enum_metadata_key(
    ...
    if trimmed.is_empty() {
        return Err(line_error(line, "enum variant metadata key cannot be empty"));
    }
...
"#;
        ...
        assert!(
            err.to_string().contains("requires `label <key>`"),
            "{err}"
        );
    }
}
