...
use crate::ast::{
    Agent, AgentEvalAssertion, AgentEvalCase, AgentEvalKind, AgentEvalPredicate, AgentInputSlot,
    AgentOutput, AgentTool, Aggregate, Command, ContainsRhs, Document, FeatureSkeleton, Field,
    FieldModifier, LzxAction, LzxApp, LzxAudience, LzxDocument, LzxExperience, LzxExperienceView,
    LzxExtensionOrder, LzxExtensionSlot, LzxPlatform, LzxPlatformView, LzxRoute, LzxSurface,
    LzxViewExtension, Query, Span, Surface, ToolsCallsOp,
};
...
fn parse_agent(
    ...
    while i < lines.len() {
        ...
        if trimmed == "input" {
            ...
            return Err(line_error(
                line,
                "agent children are `input`, `context`, `policy`, `rate_limit`, `output`, `model`, `temperature`, `max_tokens`, `top_p`, `seed`, `prompt`, `safety`, `tools`, or `evals`",
            ));
        }
