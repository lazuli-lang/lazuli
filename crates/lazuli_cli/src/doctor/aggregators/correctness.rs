...
// =============================================================================
// Tier 3 fact-driven correctness dispatchers (rows 35-39).
//
// Each helper below maps one closed-catalog rule from
// `lazuli_doctor::correctness::*` against the per-feature
// `Tier3FeatureFacts` bundle, then anchors the finding at the offending
// command / query line (falling back to the feature header when the
// per-construct line table doesn't have a match). The dispatcher in
// `dispatch.rs` invokes each in turn and concatenates the result.
