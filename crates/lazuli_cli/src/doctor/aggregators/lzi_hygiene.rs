...
use lazuli_doctor::lzi_hygiene::{
    ...
    preset::LziHygienePreset, walker::walk_lzi_sources,
};
...
mod tests {
    ...
    fn write(root: &Path, rel: &str, body: &str) {
        ...
        fs::File::create(&p).unwrap().write_all(body.as_bytes()).unwrap();
    }
    ...
    fn aggregator_fires_cohesion_on_arbitrary_bundle() {
        ...
        let diagnostics = lzi_hygiene_diagnostics(tmp.path(), None);
        assert!(diagnostics.iter().any(|d| d.code == "LZI-FEATURE-COHESION-001"));
    }
