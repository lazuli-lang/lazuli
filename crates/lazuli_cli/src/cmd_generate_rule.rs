//! `lazuli generate rule <feature>.<name>` subcommand.
//!
//! Wave 3 — appends a `rule` block with a pre-populated `tests` block
//! seeded with `@TODO authored:` markers. The markers trip
//! `TEST-STUB-001` until the author replaces them with real assertions.
...
pub fn run(ident: &str, project_root: &Path) -> Result<()> {
    ...
    if existing.contains(&needle) {
        return Err(anyhow!(
            "rule `{}.{}` already exists in {} — refusing to clobber",
            feature,
            name,
            lzi_path.display()
        ));
    ...
    let block = render_rule_block(&name);
    ...
    println!(
        "appended rule `{}.{}` with `tests` block ({} @TODO authored: markers) to {}",
        feature,
        name,
...
fn render_rule_block(name: &str) -> String {
    format!(
        "
  rule {name}
    # @TODO authored: replace placeholder predicate with real domain assertion
    requires self.status != null
    tests
      # @TODO authored: cover requires-predicate boundary
      allows when self.status != null
      denies when self.status = null
"
    )
}
...
fn parse_ident(ident: &str) -> Result<(String, String)> {
    ...
    if feature.is_empty() || name.is_empty() || parts.next().is_some() {
        return Err(anyhow!("rule ident must be <feature>.<name>; got {ident}"));
    }
...
mod tests {
    ...
    fn refuses_to_clobber_existing_rule() {
        ...
        let err = run("post.publishable", project.path()).unwrap_err();
