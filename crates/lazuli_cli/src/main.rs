...
fn write_command_sdk(
    ...
    if let Some(rate_limit) = command.rate_limit.as_ref() {
        // `ir-rate-limit-env-aware` — TS metadata exposes the unqualified
        // default; env-aware variants resolve in the runtime, not at
        // build time, so the JS shape stays as a single string.
...
fn project_auth(feature_name: &str, auth: &lazuli_ir::Auth) -> InspectAuth {
    ...
    InspectAuth {
        ...
        password: auth.password.as_ref().map(|p| InspectAuthPassword {
            ...
            // `ir-rate-limit-env-aware` — InspectAuthPassword carries
            // the unqualified default string; the inspect cell (3) will
            // surface env-aware overrides separately if needed.
            rate_limit: p.rate_limit.as_ref().map(|r| r.default.clone()),
            origin: origin.clone(),
        }),
...
mod tests {
    ...
    fn command_sdk_emits_policy_rate_limit_audit_metadata() {
        ...
        feature.commands.push(lazuli_ir::Command {
            ...
            emits: vec![],
            rate_limit: Some(lazuli_ir::RateLimitSpec::from_default("30 per hour per user")),
