...
fn emit_feature_ts_artifacts(
    ...
    if !feature.resources.is_empty() || !feature.commands.is_empty() {
        for target_prefix in feature_ts_target_prefixes(feature, manifest) {
            out.push(lazuli_codegen_ts::GeneratedFile {
                path: format!("dist/{}/{}/{}.gen.ts", target_prefix, feature.name, feature.name),
                contents: emit_feature_sdk_ts(feature, module),
            });
            out.push(lazuli_codegen_ts::GeneratedFile {
                path: format!("dist/{}/{}/{}.zod.ts", target_prefix, feature.name, feature.name),
...
fn emit_feature_sdk_ts(feature: &lazuli_ir::Feature, module: &lazuli_ir::Module) -> String {
    ...
    writeln!(s, "import {{ defineCommand, defineQuery, type ID, type Time }} from \"@lazuli/runtime\";").ok();
    writeln!(s).ok();
...
fn write_command_sdk(
    ...
    for slot in command_sdk_slots(feature, command, module) {
        ...
        writeln!(s, "  {}{}: {};", slot.name, optional, ts_type_for_type_ref(&slot.type_ref, module)).ok();
    }
    writeln!(s, "}}").ok();
    writeln!(s).ok();
...
fn write_query_sdk(
    ...
    let args_ty = if args.is_empty() {
        ...
        let fields = args
            ...
            .map(|slot| {
                let optional = if slot.required { "" } else { "?" };
                format!("{}{}: {}", slot.name, optional, ts_type_for_type_ref(&slot.type_ref, module))
            })
            .collect::<Vec<_>>()
            .join("; ");
        format!("{{ {fields} }}")
...
fn collect_input_slot_from_expr(
    ...
    if !path.segments.first().is_some_and(|segment| segment == "input") {
        return;
    }
...
fn query_input_slot(feature: &lazuli_ir::Feature, module: &lazuli_ir::Module, name: &str) -> TsSlot {
    let field = feature
        .resources
        .first()
...
fn command_output_ts_type(
    ...
    match &command.effect {
        ...
        lazuli_ir::CommandEffect::Returns(effect) => ts_type_for_type_ref(&effect.return_type, module),
...
fn ts_type_for_type_ref(type_ref: &lazuli_ir::TypeRef, module: &lazuli_ir::Module) -> String {
    match type_ref {
        lazuli_ir::TypeRef::Builtin(builtin) => match builtin {
            ...
            | lazuli_ir::BuiltinType::SemanticMoney => "number".to_owned(),
        ...
        lazuli_ir::TypeRef::EnumRef(_) => "string".to_owned(),
