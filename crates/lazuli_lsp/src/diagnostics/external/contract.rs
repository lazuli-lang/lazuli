//! Shape checks for `contract <namespace.version>` blocks
//! (external service contracts in `contracts/*.lzi`).
//!
//! Covers the header, the four canonical children (`purpose`,
//! `compatibility`, `import`, and the named blocks `record`, `operation`,
//! `event`), and the field grammar inside them. Re-exports the
//! contract-token validators (`is_contract_name`,
//! `is_contract_type_token`) and operation sub-shape recognisers
//! (`is_contract_operation_retry`, `is_contract_operation_idempotency`,
//! `is_contract_operation_error`).
